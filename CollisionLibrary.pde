...
boolean rayCircleIntersectBool(Vec3 center, float r, Vec3 l_start, Vec3 l_dir, float max_t){
...
hitInfo rayCircleListIntersect(Vec3[] centers, float[] radii,  int numObstacles, Vec3 l_start, Vec3 l_dir, float max_t, float agentRad){
  hitInfo hit = new hitInfo();
  hit.t = max_t;
  for (int i = 0; i < numObstacles; i++){
    Vec3 center = centers[i];
    ...
    hitInfo circleHit = rayCircleIntersect(center, r + agentRad, l_start, l_dir, hit.t);
